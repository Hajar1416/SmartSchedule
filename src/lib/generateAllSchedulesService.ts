...
export class GenerateAllSchedulesService {
  ...
  static async generateLevelSchedule(level: number, specificGroups?: string[], skipDatabaseSave?: boolean): Promise<GeneratedSchedule> {
    try {
      ...
      // Filter out placeholder elective codes like CS2EL7, CS2EL8, etc.
      const isPlaceholderElectiveCode = (code: string): boolean => /EL\d+$/i.test(code) || /^[A-Z]{2,}\d*EL\d+$/i.test(code)
      const filteredCourses = (courses as any[]).filter(c => {
        const t = (c.course_type || '').toString().toLowerCase()
        if (t === 'elective' && isPlaceholderElectiveCode(c.code)) return false
        return true
      })
      if (filteredCourses.length !== courses.length) {
        console.log(`🧹 Removed ${courses.length - filteredCourses.length} placeholder elective codes from allowed course list`)
      }
      ...
      // FINAL GUARANTEE: Ensure every course (compulsory + elective) appears at least once across groups
      try {
        const allRooms = [
          'A101', 'A102', 'A103', 'A104', 'A105', 'A106',
          'B201', 'B202', 'B203', 'B204', 'B205', 'B206',
        ...
        // Generate time slots with proper 50 and 100 minute durations
        const baseTimeSlots = ['08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00']
        const timeSlots: Array<{ start: string; end: string }> = []
        for (const startTime of baseTimeSlots) {
          // Add 50-minute slot
          timeSlots.push({
            start: startTime,
            end: this.calculateEndTime(startTime, 50)
          })
          // Add 100-minute slot
          timeSlots.push({
            start: startTime,
            end: this.calculateEndTime(startTime, 100)
          })
        }
        const overlapsBreak = (start: string, end: string): boolean => {
          const breakStart = '12:00'
        ...
        const globalRoomSlots = new Set<string>() // room-day-start
        const groupDayStart: Record<string, Set<string>> = {}
        for (const [gName, g] of Object.entries(groups)) {
          groupDayStart[gName] = new Set<string>()
          for (const s of (g as any).sections || []) {
            globalRoomSlots.add(`${s.room}-${s.day}-${s.start_time}`)
            groupDayStart[gName].add(`${s.day}-${s.start_time}`)
          }
        }

        const presentCodes = new Set<string>()
        for (const g of Object.values(groups)) {
          for (const s of (g as any).sections || []) presentCodes.add(s.course_code)
        }
        const requiredCodes = filteredCourses.map(c => c.code)
        const missingCodes = requiredCodes.filter(code => !presentCodes.has(code))

        // Helper: choose group with fewest sections
        const pickTargetGroup = (): [string, any] => {
          const entries = Object.entries(groups)
          entries.sort((a, b) => ((a[1] as any).sections?.length || 0) - ((b[1] as any).sections?.length || 0))
          return entries[0]
        }

        for (const code of missingCodes) {
          const course = (courses as any[]).find(c => c.code === code)
          if (!course) continue
          const isLab = !!course.is_lab || (course.code || '').toUpperCase().includes('L') && (course.title || '').toLowerCase().includes('lab')
          let placed = false
          const [targetGroupName, targetGroup] = pickTargetGroup()
          for (let i = 0; i < days.length * timeSlots.length && !placed; i++) {
            const day = days[i % days.length]
            const slot = timeSlots[i % timeSlots.length]
            if (overlapsBreak(slot.start, slot.end)) continue
            const groupKey = `${day}-${slot.start}`
            if (groupDayStart[targetGroupName].has(groupKey)) continue
            for (const room of allRooms) {
              const roomKey = `${room}-${day}-${slot.start}`
              if (globalRoomSlots.has(roomKey)) continue
              if (isLab && !room.startsWith('LAB')) continue
              if (!isLab && room.startsWith('LAB')) continue

              // Calculate end time based on course duration_minutes
              const courseDuration = course.duration_minutes || 50 // Default to 50 minutes if not specified
              const calculatedEndTime = this.calculateEndTime(slot.start, courseDuration)
              
              // Place section
              ;(targetGroup as any).sections.push({
                course_code: course.code,
                course_title: course.title,
                section_label: this.getGroupSectionLabel(targetGroupName),
                day,
                start_time: slot.start,
                end_time: calculatedEndTime,
                room,
                instructor: `Dr. ${course.code}`,
                student_count: Math.min((targetGroup as any).student_count || 25, 30),
                capacity: 30
              })
              globalRoomSlots.add(roomKey)
              groupDayStart[targetGroupName].add(groupKey)
              placed = true
              console.log(`🔵 Final guarantee placed ${course.code} in ${targetGroupName} at ${day} ${slot.start}`)
              break
            }
          }
          if (!placed) console.warn(`⚠️ Final guarantee could not place ${course.code}`)
        }
      } catch (e) {
        console.warn('Final guarantee step failed:', e)
      }

      // Calculate totals
      const totalSections = Object.values(groups).reduce((sum, group) => sum + group.sections.length, 0)
      const conflicts = this.detectConflicts(groups)  // Only check conflicts WITHIN this schedule
      ...
      // Check for inter-level conflicts ONLY (conflicts with OTHER levels, not same level's other versions)
      const interLevelConflicts = await this.detectInterLevelConflicts(schedule)
      console.log(`🔍 Detected ${interLevelConflicts} inter-level conflicts (only checking OTHER levels, not version history)`)
      ...
      if (interLevelConflicts > 0) {
        ...
        // Recalculate conflicts after resolution - ONLY internal conflicts within this version
        ...
        console.log(`✅ Conflict resolution complete. Final conflicts: ${finalSchedule.conflicts} (internal: ${resolvedConflicts}, inter-level: ${resolvedInterLevelConflicts})`)
      } else {
        console.log(`✅ No inter-level conflicts detected. Total conflicts: ${conflicts} (internal only)`)
      }
  ...
  private static async generateGroupScheduleWithAI(
    ...
    // Seed electives if no preferences were provided (ensure electives are included)
    const electivePlan = (preferenceAnalytics && preferenceAnalytics.length > 0)
      ? preferenceAnalytics
      : electiveCourses.map((c: any) => ({
          course_code: c.code,
          course_title: c.title,
          sections_needed: 1,
          student_count: 0
        }))
    if (!preferenceAnalytics || preferenceAnalytics.length === 0) {
      console.log(`🟦 No preference data; seeding baseline elective plan for ${electivePlan.length} electives (1 section each)`) 
    }
    ...
    // Note: We'll use a default duration for blocked slots since we don't have course info
    const blockedSlotsFromOtherLevels = occupiedSlots.map(slot => ({
      day: slot.day,
      start: slot.start,
      end: this.calculateEndTime(slot.start, 50), // Use 50 minutes as default for blocked slots
      room: slot.room
    }))
    ...
    const constraints: SchedulingConstraints = {
      ...
      rules: [
        ...
        // 🔒 COURSE LIST CONSTRAINTS (prevent invented courses)
        `Allowed courses ONLY (do NOT invent new codes): ${courses.map(c => `${c.code} (${c.title})`).join(', ')}`,
        'NEVER output a course that is not in the Allowed courses list.',
        // 🆕 Add elective constraints (preferences if available, otherwise baseline)
        '\n🎯 ELECTIVE COURSES:',
        ...electivePlan.map(pref => 
          `- Generate ${pref.sections_needed} section(s) of "${pref.course_title}" (${pref.course_code})` +
          (pref.student_count ? ` because ${pref.student_count} students requested it` : ' (baseline)')
        )
      ],
    ...
    try {
      ...
      const allowedCourseCodes = new Set<string>(courses.map(c => c.code))
      const courseByCode = new Map<string, any>(courses.map(c => [c.code, c]))
      
      for (const rec of recommendations) {
        // Skip any recommendation that uses unknown course codes
        if (!allowedCourseCodes.has(rec.course_code)) {
          console.warn(`⛔ Skipping unknown course from AI: ${rec.course_code} (not in allowed list) for ${groupName}`)
          continue
        }
      ...
      // Post-process: replace generic elective placeholders with actual elective course codes
      const electiveTargetCodes: string[] = electivePlan
        .map(p => p.course_code)
        .filter(code => allowedCourseCodes.has(code))

      const isPlaceholderElective = (code: string): boolean => /EL\d+$/i.test(code) || /(^|\b)[A-Z]{2,}\d*EL\d+$/i.test(code)

      const alreadyUsed = new Set<string>(sections.map(s => s.course_code))
      const availableElectiveQueue = electiveTargetCodes.filter(c => !alreadyUsed.has(c))

      for (let i = 0; i < sections.length; i++) {
        const s = sections[i]
        const course = courseByCode.get(s.course_code)
        const isElective = (course?.course_type || '').toString().toLowerCase() === 'elective' || isPlaceholderElective(s.course_code)
        if (isElective && (isPlaceholderElective(s.course_code) || !electiveTargetCodes.includes(s.course_code))) {
          const replacement = availableElectiveQueue.shift()
          if (replacement && allowedCourseCodes.has(replacement)) {
            console.log(`♻️ Replacing placeholder elective ${s.course_code} with ${replacement} for ${groupName}`)
            s.course_code = replacement
            s.course_title = courseByCode.get(replacement)?.title || replacement
            alreadyUsed.add(replacement)
          }
        }
      }

      // Ensure ALL compulsory courses are present (AI may return electives heavily)
      const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday']
      // Generate time slots with proper 50 and 100 minute durations
      const baseTimeSlots = ['08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00']
      const timeSlots: Array<{ start: string; end: string }> = []
          ...
          for (const room of candidateRooms) {
            if (!usedGroupSlots.has(key) && this.isRoomAvailableGlobally(room, day, slot.start)) {
              ...
              sections.push({
                ...
                day,
                start_time: slot.start,
                end_time: this.calculateEndTime(slot.start, course.duration_minutes || 50),
                room,
                instructor: `Dr. ${course.code}`,
      ...
      // Ensure AT LEAST ONE section for EVERY ELECTIVE course at this level
      // This guarantees electives are available in the schedule even without preferences
      let electiveOffset = 0
      for (const course of electiveCourses) {
        if (usedCourseCodes.has(course.code)) continue
        let placed = false
        for (let i = 0; i < days.length * timeSlots.length; i++) {
          const day = days[(i + electiveOffset) % days.length]
          const slot = timeSlots[(i + electiveOffset) % timeSlots.length]
          const key = `${day}-${slot.start}`
          const isLab = !!course.is_lab || (course.code || '').toUpperCase().includes('LAB') || (course.title || '').toLowerCase().includes('lab')
          const candidateRooms = allRooms.filter(r => isLab ? r.startsWith('LAB') : !r.startsWith('LAB'))
          for (const room of candidateRooms) {
        ...
        if (!placed) console.warn(`⚠️ Could not place elective course ${course.code} for ${groupName}`)
        electiveOffset++
      }
  ...
  private static generateFallbackSchedule(
    ...
    // Enforce system policy: Monday-Thursday with proper durations
    const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday']
    const baseTimeSlots = [
      '08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00'
    ]
    
    // Generate time slots based on course durations
    const timeSlots: Array<{ start: string; end: string }> = []
    for (const startTime of baseTimeSlots) {
      // Add 50-minute slot
      timeSlots.push({
  ...
  private static async detectInterLevelConflicts(newSchedule: GeneratedSchedule): Promise<number> {
    try {
      ...
      // Get only ACTIVE schedules from other levels to avoid false conflicts with historical versions
      const { data: existingSchedules } = await supabase
        .from('schedule_versions')
        .select('*')
        .neq('level', newSchedule.level)
        .eq('is_active', true)  // Only check active versions

      if (!existingSchedules || existingSchedules.length === 0) {
        return 0 // No other levels to conflict with
      }

      console.log(`🔍 Checking for conflicts with ${existingSchedules.length} active schedule(s) from other levels`)
        ...
        for (const section of sections) {
          ...
          while (attempts < maxAttempts) {
            ...
            resolvedSection = {
              ...
              end_time: this.calculateEndTime(randomTime, 50), // Use 50 minutes as default for conflict resolution
              room: randomRoom
            }
  ...
  static async saveSingleScheduleToDatabase(schedule: GeneratedSchedule): Promise<string | null> {
    try {
      ...
      const activeVersion = await ScheduleVersionService.getActiveVersion(schedule.level, semester)
      ...
      const newVersion = await ScheduleVersionService.createVersion({
        ...
        change_description: 'Generated schedule',
        status: 'draft',
        is_active: false
      })

      console.log(`✅ Successfully created new version ${newVersion.version_number} for Level ${schedule.level} schedule with ID: ${newVersion.id}`)
  ...
  static async saveSchedulesToDatabase(schedules: GeneratedSchedule[]): Promise<void> {
    try {
      for (const schedule of schedules) {
        ...
        // Create new version using version service
        await ScheduleVersionService.createVersion({
          level: schedule.level,
          semester,
          groups: schedule.groups,
          total_sections: totalSections,
  ...
  static async updateSchedule(
    ...
    try {
      console.log('💾 Creating new version from schedule update:', {
        id: scheduleId,
      ...
      const currentVersion = await ScheduleVersionService.getVersionById(scheduleId)
      if (!currentVersion) {
      ...
      const newVersion = await ScheduleVersionService.createVersion({
        ...
        semester: currentVersion.semester,
        ...
        parent_version_id: scheduleId,
        created_by: createdBy,
        change_description: changeDescription || 'Manual schedule edit',
        status: 'draft',
        is_active: false
      })
      ...
      console.error('Error creating version from schedule update:', error)
      throw error
    }
  }
