...
export class IrregularScheduleService {
  ...
  static async getIrregularStudentSchedule(
    ...
    try {
      ...
      // Extract ALL courses from current level (compulsory + electives)
      // Show complete schedule just like regular students and admin see it
      ...
      if (levelSchedule && levelSchedule.groups) {
        ...
        // Fetch course details to get titles, types, and credits
        // Don't filter by level here - get all courses that are in the schedule
        const { data: coursesData, error: coursesError } = await supabase
          .from('courses')
          .select('code, title, course_type, credits, level')
          .in('code', Array.from(allCourseCodes))
        
        console.log(`🔍 Fetching course details for ${allCourseCodes.size} courses:`, Array.from(allCourseCodes))
        
        if (!coursesError && coursesData) {
          // Create maps for course information
          ...
          // Include ALL sections from current level (both compulsory and elective)
          Object.values(levelSchedule.groups).forEach((group: any) => {
            (group as any).sections?.forEach((section: any) => {
              ...
              if (!seenSections.has(sectionKey)) {
                ...
                // Determine source based on course type
                const source = courseType === 'compulsory' ? 'current' : 'elective'
          ...
          console.log(`✅ Loaded ${currentLevelSections.length} total sections for level ${student.level}`)
          console.log(`   - Compulsory: ${currentLevelSections.filter(s => s.course_type === 'compulsory').length}`)
          console.log(`   - Elective: ${currentLevelSections.filter(s => s.course_type === 'elective').length}`)
        }
      }

      // Get failed courses list to identify them in preferences
      const { data: failedCourseRequirements, error: failedError } = await supabase
        .from('irregular_course_requirements')
        .select(`
          *,
          course:courses(code, level, title)
        `)
        .eq('student_id', studentId)
      
      const failedCourseCodes = new Set(
        (failedCourseRequirements || [])
          .map((req: any) => req.course?.code)
          .filter(Boolean)
      )
      
      console.log(`📚 Failed course codes:`, Array.from(failedCourseCodes))
      console.log(`📚 Failed course details:`, (failedCourseRequirements || []).map((req: any) => ({
        code: req.course?.code,
        title: req.course?.title,
        level: req.course?.level,
        original_level: req.original_level
      })))
      ...
      if (preferences && preferences.length > 0) {
        // Get unique levels from preferences that are FAILED COURSES ONLY (exclude current level)
        // Current level electives are already shown in the main schedule
        const failedCoursePreferences = preferences.filter((p: any) => 
          failedCourseCodes.has(p.course?.code)
        )
        
        const preferenceLevels = [...new Set(
          failedCoursePreferences
            .map((p: any) => p.course?.level)
            .filter((level: number) => level && level !== student.level) // Only other levels
        )]
        
        console.log(`📋 Failed course preferences:`, failedCoursePreferences.map(p => p.course?.code))
        console.log(`📋 Preference levels to load (failed courses only):`, preferenceLevels)
        console.log(`📋 Student current level:`, student.level)
        ...
        if (preferenceLevels.length > 0) {
          ...
          console.log(`🔍 Querying schedule_versions for levels:`, preferenceLevels, `semester:`, semester)
          console.log(`📦 Query result:`, { 
            error: prefSchedError, 
            scheduleCount: preferenceSchedules?.length || 0,
            schedules: preferenceSchedules?.map(s => ({ level: s.level, id: s.id, hasGroups: !!s.groups }))
          })

          if (!prefSchedError && preferenceSchedules) {
            // Get only FAILED course codes that student selected
            const selectedFailedCourseCodes = preferences
              .filter((p: any) => failedCourseCodes.has(p.course?.code))
              .map((p: any) => p.course?.code)
              .filter(Boolean)
            
            console.log(`📋 Student selected ${selectedFailedCourseCodes.length} failed courses:`, selectedFailedCourseCodes)
            
            // Helper function to check if course codes match (exact or partial)
            const courseCodesMatch = (scheduleCode: string, requiredCode: string): boolean => {
              // Exact match
              if (scheduleCode === requiredCode) return true
              
              // Remove spaces and compare
              const normalizedScheduleCode = scheduleCode.replace(/\s+/g, '')
              const normalizedRequiredCode = requiredCode.replace(/\s+/g, '')
              if (normalizedScheduleCode === normalizedRequiredCode) return true
              
              // Check if schedule code starts with required code (e.g., MATH151T starts with MATH151)
              if (normalizedScheduleCode.startsWith(normalizedRequiredCode)) return true
              
              // Check if required code starts with schedule code (e.g., MATH151 starts with MATH)
              // This is more lenient but helps with course variations
              const requiredBase = normalizedRequiredCode.replace(/[LT]$/, '') // Remove L or T suffix
              const scheduleBase = normalizedScheduleCode.replace(/[LT]$/, '')
              if (requiredBase === scheduleBase) return true
            ...
            // Extract ONLY sections for the student's selected failed courses
            preferenceSchedules.forEach((schedule: any) => {
              console.log(`📚 Checking schedule for level ${schedule.level}:`)
              console.log(`  - Has groups:`, !!schedule.groups)
              if (schedule.groups) {
                const allCoursesInSchedule = new Set<string>()
                Object.entries(schedule.groups).forEach(([groupName, group]: [string, any]) => {
                  ...
                  groupSections.forEach((section: any) => {
                    allCoursesInSchedule.add(section.course_code)
                  })
                })
                console.log(`  - All course codes in this schedule:`, Array.from(allCoursesInSchedule))
                console.log(`  - Looking for:`, selectedFailedCourseCodes)
                
                // Find matches using fuzzy logic
                const matchedCourses: any[] = []
                selectedFailedCourseCodes.forEach(requiredCode => {
                  const matches = Array.from(allCoursesInSchedule).filter(scheduleCode => 
                    courseCodesMatch(scheduleCode, requiredCode)
                  )
                  if (matches.length > 0) {
                    matchedCourses.push({ required: requiredCode, found: matches })
                  }
                })
                console.log(`  - Matched courses:`, matchedCourses)
                console.log(`  - Missing courses:`, selectedFailedCourseCodes.filter(code => 
                  !Array.from(allCoursesInSchedule).some(scheduleCode => courseCodesMatch(scheduleCode, code))
                ))
                ...
                Object.values(schedule.groups).forEach((group: any) => {
                  ...
                  groupSections.forEach((section: any) => {
                    // ONLY include if this section matches one of the student's failed courses
                    const isMatch = selectedFailedCourseCodes.some(requiredCode => 
                      courseCodesMatch(section.course_code, requiredCode)
                    )
                    if (isMatch) {
                      // Create unique key for this section
                      const sectionKey = `${section.course_code}-${section.section_label}-${section.day}-${section.start_time}`
                      ...
                      if (!seenPreferenceSections.has(sectionKey)) {
                        ...
                        // Get original level for failed courses
                        const failedCourse = (failedCourseRequirements || []).find(
                          (req: any) => req.course?.code === section.course_code
                        )
                        const originalLevel = failedCourse?.original_level
            ...
            console.log(`✅ Found ${preferenceSections.length} failed course sections from previous levels`)
            console.log(`✅ Failed courses to retake:`, preferenceSections.map(s => ({
              course: s.course_code,
              original_level: s.original_level,
              current_level: s.course_level,
              day: s.day,
