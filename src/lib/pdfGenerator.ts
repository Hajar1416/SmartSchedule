...
// Generate multi-level timetable PDF (one A4 landscape page per level)
export function generateMultiLevelTimetablePDF(
  pages: Array<{ level: number; schedule: TimetableEntry[] }>,
  studentInfo: { name?: string; studentNumber?: string; semester?: string } = {}
): void {
  const doc = new jsPDF('landscape', 'mm', 'a4')
  ...
  pages.forEach((page, idx) => {
    ...
    doc.setFontSize(20)
    doc.setFont('helvetica', 'bold')
    doc.text(`Class Timetable - Level ${page.level}`, doc.internal.pageSize.getWidth() / 2, 20, { align: 'center' })
    ...
    // Build table
    const timeSlots = deriveTimeSlots(page.schedule as any)
    const tableData: string[][] = []
    const headerRow = ['Time', ...DAYS]
    ...
    timeSlots.forEach(timeSlot => {
      const row = [timeSlot]
      DAYS.forEach(day => {
        const entries = page.schedule.filter((entry: any) => {
          const entryDay = entry?.timeslot?.day || entry?.day
          const entryStart = entry?.timeslot?.start || entry?.start_time
          return entryDay === day && formatTime12Hour(entryStart) === timeSlot
        })
  ...
  const filename = `timetable_irregular_${new Date().toISOString().split('T')[0]}.pdf`
  doc.save(filename)
}
