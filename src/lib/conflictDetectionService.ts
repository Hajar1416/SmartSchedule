...
export class ConflictDetectionService {
   ...
   * Only checks ACTIVE versions to avoid false conflicts with historical versions
   */
  static async detectAllConflicts(): Promise<Conflict[]> {
    try {
      const conflicts: Conflict[] = []

      // Get only ACTIVE schedule versions to avoid false conflicts with historical versions
      ...
      console.log(`🔍 Checking conflicts for ${schedules.length} active schedule(s)`)
  ...
  private static detectTimeConflicts(schedules: any[]): Conflict[] {
    ...
    schedules.forEach(schedule => {
      // Use groups directly (new schema) or fallback to diff_json.groups (old schema)
      const groups = schedule.groups || schedule.diff_json?.groups || {}
