...
export class ScheduleService {
  static async generateSchedule(request: ScheduleGenerationRequest): Promise<GeneratedSchedule> {
    try {
      ...
      try {
        ...
        recommendations = flexibleCourses.map((course, index) => ({
          ...
          timeslot: {
            ...
            start: '14:00',
            end: this.calculateEndTime('14:00', course.duration_minutes || 50)
          },
      ...
      const fixedSections: ScheduleSection[] = fixedCourses.map((course, index) => {
        ...
        const endTime = this.calculateEndTime(startTime, course.duration_minutes || 50)
  ...
  static async getSchedules(filterByActive: boolean = true): Promise<GeneratedSchedule[]> {
    let query = supabase
      .from('schedule_versions')
      .select('*')
    ...
    const mapped: GeneratedSchedule[] = (data || [])
      ...
      .map((row: any) => {
        ...
        return {
          ...
          sections,
          conflicts: row?.diff_json?.conflicts || row.conflicts || 0,
          efficiency: row?.diff_json?.efficiency || row.efficiency || 0,
          status: ((row?.diff_json?.status) || row.status || 'draft') as 'draft' | 'approved' | 'active',
          created_at: row.created_at,
          author_id: row.created_by || row.author_id,
          approved_at: row?.diff_json?.approved_at || row.approved_at
        }
      })
  ...
  static async approveSchedule(scheduleId: string, approvedBy: string): Promise<void> {
    await ScheduleVersionService.approveVersion(scheduleId, approvedBy)
  }
   ...
   */
  static async updateScheduleSections(
    scheduleId: string, 
    sections: ScheduleSection[],
    createdBy?: string,
    ...
    // Convert sections to groups format
    const existingGroups = currentVersion.groups || {}
    const grouped: any = {}
    for (const s of sections) {
    ...
    // Use existing efficiency and conflicts for now (can be recalculated later)
    const conflicts = currentVersion.conflicts || 0
    const efficiency = currentVersion.efficiency || null
    ...
    const newVersion = await ScheduleVersionService.createVersion({
      ...
      change_description: changeDescription || 'Schedule sections updated',
      status: 'draft',
      is_active: false
    })
