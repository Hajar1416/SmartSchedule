  ...
  static async getRulesAsAIConstraints(): Promise<string[]> {
    ...
    for (const rule of activeRules) {
      switch (rule.rule_type) {
        ...
        case 'lab_continuous':
          constraints.push(
            `REQUIREMENT: Lab courses (marked as is_lab) MUST be scheduled as continuous ${rule.parameters.duration_minutes}-minute blocks. No breaks between lab time slots.`
          )
          break
