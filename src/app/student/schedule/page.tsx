...
import { generateTimetablePDF, generateMultiLevelTimetablePDF } from '@/lib/pdfGenerator'
...
export default function StudentSchedulePage() {
  ...
  const loadIrregularStudentSchedule = async (studentData: any) => {
    try {
      ...
      console.log('📊 Irregular Schedule Result:', result)
      ...
      if (result) {
        ...
        if (result.currentLevelSections && result.currentLevelSections.length > 0) {
          ...
          console.log(`✅ Added ${result.currentLevelSections.length} sections to level ${level}`)
        }
        ...
        if (result.preferenceSections && result.preferenceSections.length > 0) {
          console.log(`📋 Processing ${result.preferenceSections.length} preference sections:`)
          result.preferenceSections.forEach((section: any) => {
            const level = section.course_level || section.level
            console.log(`  - ${section.course_code} belongs to level ${level}`)
            
            // Skip if this is the current level (already included in currentLevelSections)
            if (level === studentData.level) {
              console.log(`  ⏭️ Skipping ${section.course_code} (current level)`)
              return
            }
            
            if (!grouped[level]) {
              grouped[level] = []
              console.log(`  ✅ Created new group for level ${level}`)
            }
            grouped[level].push(section)
            console.log(`  ✅ Added ${section.course_code} to level ${level} group`)
          })
        }
        
        console.log('📊 Final grouped sections by level:', Object.keys(grouped).map(level => ({
          level,
          count: grouped[Number(level)].length,
          courses: grouped[Number(level)].map((s: any) => s.course_code)
        })))
        ...
        console.log('❌ No schedule result returned')
        setSectionsByLevel({})
  ...
  const handleDownloadPDF = () => {
    ...
    // Irregular: export each level shown to the student as a separate PDF page
    if (student.is_irregular) {
      const pages = Object.keys(sectionsByLevel)
        .sort((a, b) => Number(a) - Number(b))
        .map(levelStr => {
          const level = Number(levelStr)
          const sections = sectionsByLevel[level] || []
          const schedule = sections.map((entry: any) => ({
      ...
      generateMultiLevelTimetablePDF(pages, {
