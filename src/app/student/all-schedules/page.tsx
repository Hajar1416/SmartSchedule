...
// 1. IMPORT the ScheduleCommentsPanel component
import { ScheduleCommentsPanel } from '@/components/ui/ScheduleCommentsPanel'
import { MessageSquare } from 'lucide-react' // Added icon for the placeholder
...
  const [scheduleVersion, setScheduleVersion] = useState<any>(null) // This contains the ID
...
  return (
    // Wrapped in a layout div to accommodate the comments sidebar
    <div className="container mx-auto py-8 px-4">
        {/* Main Content - Schedule + Comments Side by Side (Added Grid Layout) */}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">

            {/* Schedule Section (3/4 width) */}
            ...
            {/* 3. Render Comments Panel (1/4 width) */}
            <div className="lg:col-span-1">
                {scheduleVersion && scheduleVersion.id ? (
                    <ScheduleCommentsPanel
                        // Pass the schedule ID from the loaded version
                        scheduleVersionId={scheduleVersion.id}
                        // Note: If you need to scope comments per group, you would pass selectedGroup as well, 
                        // and modify ScheduleCommentsPanel to handle that context.
                    />
                ) : (
                    <Card>
                        <CardContent className="text-center py-8">
                            ...
                            <p className="text-gray-600 text-sm">
                                Comments will be available once the full schedule version is published.
                            </p>
                        </CardContent>
                    </Card>
                )}
