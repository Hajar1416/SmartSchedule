...
import { VersionComparisonView } from '@/components/ui/VersionComparisonView'
 ...
 const { participants } = useCollaborationParticipantsForSchedule(
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      // Validate and save to DB via service - creates new version
      const newVersionId = await ScheduleService.updateScheduleSections(
        selectedSchedule.id,
        sections as any,
        user?.id,
      ...
      // Reload schedules to get the new version
      await loadSchedules()
      
      // Select the new version
      setSelectedVersionId(newVersionId)
      const updatedSchedule = await ScheduleService.getScheduleById(newVersionId)
      if (updatedSchedule) {
        setSelectedSchedule(updatedSchedule)
        setSections(updatedSchedule.sections.map((section, index) => ({
          ...(section as any),
          id: `section-${newVersionId}-${index}-${Date.now()}`
        })))
      }
      
      setSuccess('Schedule saved successfully! New version created.')
