...
export default function GenerateAllSchedulesPage() {
  ...
  const [selectedScheduleForComments, setSelectedScheduleForComments] = useState<string | null>(null)
    ...
    try {
      ...
      const allSchedules: GeneratedSchedule[] = (data || []).map(row => {
        ...
        return {
          ...
          level: row.level || 1,
          semester: (row as any).semester || currentSemester || '',
          groups,
    ...
    try {
      ...
      for (const level of levelsWithConflicts) {
        ...
        if (existingDbSchedules && existingDbSchedules.length > 0) {
          ...
          }, user?.id, 'AI conflict resolution')
  ...
  const downloadSchedule = (schedule: GeneratedSchedule) => {
    if (!schedule.groups) {
      console.error('Cannot download schedule: groups is undefined')
      return
    }
      ...
      if (scheduleToUpdate) {
        try {
          ...
          if (existingDbSchedules && existingDbSchedules.length > 0) {
            ...
            }, user?.id, 'AI-assisted schedule update')
  ...
  return (
      <div className="space-y-6">
        ...
        <Card>
          ...
          /* Existing Schedules Display - Combined Groups with Comments Panel */
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Schedules Column - 2/3 width */}
              ...
              <Card key={`schedule-${schedule.id || `temp-${schedule.level}-${index}`}`} className="mb-6">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        {schedule.groups ? Object.keys(schedule.groups).length : 0} groups • {schedule.total_sections || 0} total sections
                      </CardDescription>
                    </div>
                    <div className="flex items-center gap-2">
                      {schedule.id && currentSemester && (
                        <VersionSelector
                          level={schedule.level}
                          semester={schedule.semester || currentSemester}
                          value={selectedVersions[schedule.id] || schedule.id}
                          onValueChange={async (versionId) => {
                            setSelectedVersions(prev => ({
                              ...prev,
                              [schedule.id!]: versionId
                            }))
                            // Load the selected version schedule
                            if (versionId !== schedule.id) {
                              const versionSchedule = await ScheduleService.getScheduleById(versionId)
                              if (versionSchedule) {
                                // Convert GeneratedSchedule format (with sections) to the format expected by this page
                                // Need to get the full version data with groups
                                const versionData = await ScheduleVersionService.getVersionById(versionId)
                                if (versionData && versionData.groups) {
                                  const convertedSchedule: GeneratedSchedule = {
                                    id: versionData.id,
                                    level: versionData.level,
                                    semester: versionData.semester || currentSemester || '',
                                    groups: versionData.groups,
                                    total_sections: versionData.total_sections,
                                    conflicts: versionData.conflicts,
                                    efficiency: versionData.efficiency || 0,
                                    generated_at: versionData.generated_at || versionData.created_at
                                  }
                                  // Update the schedule in the list
                                  setExistingSchedules(prev => prev.map(s => 
                                    s.id === schedule.id ? convertedSchedule : s
                                  ))
                                }
                              }
                            } else {
                      ...
                      <Button
                        variant={selectedScheduleForComments === schedule.id ? "default" : "outline"}
                        size="sm"
                        onClick={() => setSelectedScheduleForComments(schedule.id === selectedScheduleForComments ? null : schedule.id || null)}
                      >
                        <MessageSquare className="h-4 w-4 mr-1" />
                        {selectedScheduleForComments === schedule.id ? 'Hide Comments' : 'View Comments'}
                      </Button>
                ...
                <CardContent>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-2">
                        {schedule.groups ? Object.keys(schedule.groups).map(groupName => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <Badge variant="secondary" className="ml-2">
                                {schedule.groups[groupName]?.student_count || 0}
                              </Badge>
                            </Button>
                          )
                        }) : null}
                      ...
                      <div className="flex gap-2">
                        {schedule.groups ? Object.entries(schedule.groups)
                          .filter(([groupName]) => {
                            const currentSelectedGroup = getSelectedGroupForSchedule(schedule.id || '')
                    ...
                    {getSelectedGroupForSchedule(schedule.id || '') === 'all' ? (
                      ...
                      schedule.groups ? (
                        ...
                        <div className="text-center py-8 text-gray-500">
                          No schedule data available for this version
                        </div>
                      )
                    ) : (
            ...
            {/* Comments Panel Column - 1/3 width */}
            <div className="lg:col-span-1">
              <div className="sticky top-6">
                {selectedScheduleForComments ? (
                  <ScheduleCommentsPanel scheduleVersionId={selectedScheduleForComments} />
                ) : (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                      ...
                      <CardDescription>
                        Click "View Comments" on any schedule to see student and faculty feedback
                      </CardDescription>
                    </CardHeader>
