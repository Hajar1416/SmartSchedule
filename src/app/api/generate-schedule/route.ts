...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: coursesData } = await supabase
      .from('courses')
      .select('code, title, level, course_type, is_lab, duration_minutes, allowable_rooms, required_hours')
      .eq('level', level)
...
- ALL courses must be scheduled for EXACTLY their specified duration_minutes
- 50-minute courses: Schedule from start_time to start_time + 50 minutes (e.g., 08:00-08:50)
- 100-minute courses: Schedule from start_time to start_time + 100 minutes (e.g., 08:00-09:40)
- Lab courses (is_lab: true) are typically 100 minutes
- Regular courses are typically 50 minutes
- Calculate end times by adding duration_minutes to start_time
...
Example response format (with correct durations):
[
  {
    "course_code": "CS2EL1",
    "section_label": "A",
    ...
    "justification": "50-minute course ending before mandatory break period (12:00-13:00)",
  ...
  },
  {
    ...
    "justification": "100-minute lab course starting after mandatory break period (12:00-13:00)",
    "confidence_score": 0.95
  }
]
    `
